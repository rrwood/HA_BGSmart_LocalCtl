...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def _async_update_data():
        """Fetch params from the device; raise UpdateFailed on failure."""
        params = await device.get_params()
        if not params:
            raise UpdateFailed(f"No params from {host}:{port}")
        return params
    ...
    # Initial refresh — raises ConfigEntryNotReady on UpdateFailed so HA retries
